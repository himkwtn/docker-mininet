...
class LearningSwitch (object):
  ...
  def CheckRule (self, dpidstr, src=0):
    try:
      ...
      action = 'Forwarding packet' if entry else 'Dropping packet'
      log.info("Firewall rule for (%s, %s) = %s. %s" % (dpidstr,src,entry,action))
      return entry
    except KeyError:
      log.info("Firewall rule not found. Dropping packet")
      return False
  ...
  def _handle_PacketIn (self, event):
    ...
    #***Added: Get the DPID of the switch connection
